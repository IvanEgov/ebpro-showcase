...
export default function ScreenshotGallery({ screenshots, projectTitle }: ScreenshotGalleryProps) {
  ...
  return (
    <>
      <button
        onClick={openModal}
        className="w-full mt-5 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors duration-200 flex items-center justify-center gap-2"
      >
        ...
        >
          ...
          >
            <div className="w-full flex items-center justify-between mb-4">
              <div>
                <h3 className="text-white text-lg md:text-xl font-semibold">{projectTitle}</h3>
              ...
              <button
                onClick={closeModal}
                className="text-white hover:text-gray-300 p-2 hover:bg-white/10 rounded-lg"
              >
                ...
                {screenshots.map((_, index) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${
                      index === currentIndex ? 'bg-blue-500 w-8' : 'bg-gray-600 w-2'
                    }`}
                  />
                ))}
              </div>
            ...
            <p className="text-gray-500 text-xs mt-3">← → для навигации • Esc для закрытия</p>
          </div>
        </div>
      )}
    </>
